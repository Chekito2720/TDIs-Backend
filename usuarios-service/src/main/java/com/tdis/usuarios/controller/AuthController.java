...
public class AuthController {
    ...
    @PostMapping("/register-interno")
    public ResponseEntity<?> registerInterno(@Valid @RequestBody RegisterInternoRequest request) {
        try {
            LoginResponse response = usuarioService.registerInterno(request);
            return ResponseEntity.status(HttpStatus.CREATED).body(response);
        } catch (BadRequestException e) {
            return ResponseEntity.badRequest().body(
                java.util.Map.of("message", e.getMessage())
